...
# class - the index in the probablity vector 
logloss = function(probability, class) {
  return(- 1 * log2(probability[, class]))
}

n_train <- c(25, 50, 100, 200, 400, 800, 1600, 3200, 6400, 12800, 25600, 51200, 51200 * 2, 51200 * 4)
...
for(i in seq_along(n_train)) {
  ...
  test_error_ML[i] <- sum(logloss(predict(nbfit_ML, test$X, probabilities = TRUE), test$Y)) / n_test
  test_error_KT[i] <- sum(logloss(predict(nbfit_KT, test$X, probabilities = TRUE), test$Y)) / n_test
  test_error_Laplace[i] <- sum(logloss(predict(nbfit_Laplace, test$X, probabilities = TRUE), test$Y)) / n_test
  test_error_logistic[i] <- sum(logloss(predict(logistic_fit, data.frame(X1 = factor(test$X[, 1]), X2 = factor(test$X[, 2])), type="probs"), test$Y)) / n_test
}
  
plot(x = n_train, log='x', test_error_Laplace, type = 'b', col = 'blue', lwd = 2, pch = 20, xlab = 'n', ylab = 'test error')
lines(x = n_train, test_error_ML, type = 'b', col = 'red', lwd = 2, pch = 20)
lines(x = n_train, test_error_KT, type = 'b', col = 'green', lwd = 2, pch = 20)
lines(x = n_train, test_error_logistic, type = 'b', col = 'black', lwd = 2, pch = 20)
