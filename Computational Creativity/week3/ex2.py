...
all_triples = factgen.read_triples("reverb.txt", min_attested=2) # format: (LHS, predicate, RHS)
entities = factgen.collect_entities(all_triples)

sample_size = 1000
selected_entities = random.sample(entities, sample_size)

facts = list(filter(lambda x: x[0] in selected_entities or x[2] in selected_entities, all_triples))

for fact in facts:
    print(fact)

# 2
generate_from_lhs("tattoos", all_triples)