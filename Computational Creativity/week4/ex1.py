...
from generation import factgen, figurative_sentence_generator, made_up_fact_generator, rhyming_pair_chooser, tostringconverter, utilities, text_from_markov_chain_generator
...
def couplet(*args, **kwargs):
    ...
    :param kwargs: The list of optional arguments as described below.
        max_words_per_line: The length of the lines in words. Must be an integer greater than 10. Defaults to 10.
    ...
    :exception: AssertionError if the max_words_per_line parameter is not an integer or is less than 10.
    ...
    max_words_per_line = kwargs.get("max_words_per_line", 10)
    assert isinstance(max_words_per_line, int) and max_words_per_line >= 10
    ...
    if verbose: print("Markov model trained with {} states".format(len(markov_model.keys())))

    if verbose: print("Generating couplet...")
    couplet = __generate_couplet(topics=args, triples=triples, markov_model=markov_model, figurative_sentence_template=figurative_sentence_template, max_line_length=max_words_per_line, verbose=verbose)
...
def __generate_couplet(topics, triples, markov_model, figurative_sentence_template, max_line_length, verbose):
    ...
    markov_text = text_from_markov_chain_generator.generate(markov_model, max_line_length)
    markov_text = tostringconverter.format_markov_chain_sentence(markov_text)
    first_line = " ".join([fact, markov_text])

    #----#
    if verbose: print("\n1st line:\n{}\n------\nCreating 2nd line...".format(first_line))

    targets = rhyming_pair_chooser.get_rhyming_noun_pairs_from_line(first_line, verbose)
...
def __gather_sentences(topic, template, verbose):
    ...
    if verbose and figurative_sentences is not None: print("Generated {} figurative sentences".format(len(figurative_sentences)))
...
#c = couplet("dinner", "cat", "dog", verbose=True, markov_chain_order=1)
#print("----")
#print(c)
