...
def generate(state_transition_probabilities, length=10, start=None):
    ...
    try:
        ...
        current_state = random.choice(list(state_transition_probabilities.keys()))  # the generation is started from a random state if start is not found
    ...
    while(len(string_list) < length): # The piece of text should be at most length tokens long
        ...
        except: # cannot proceed from the state
            current_state = random.choice(list(state_transition_probabilities.keys()))
